...
export async function fetchRequirements(
  ...
  if (validationType === "assessment" || validationType === "full_validation") {
    console.log(
      `[Requirements Fetcher] Fetching ALL requirements for ${unitCode}`,
    );
  ...
  switch (validationType) {
    ...
    case "assessment_conditions":
      ...
      try {
        console.log(
          `[Requirements Fetcher] Fetching assessment_conditions from UnitOfCompetency for ${unitCode}`,
        );
        ...
        if (uocError) {
          console.error(
            `[Requirements Fetcher] Error fetching from UnitOfCompetency:`,
            uocError,
          );
          return [];
        }
        ...
        console.warn(
          `[Requirements Fetcher] No assessment conditions found in UnitOfCompetency for ${unitCode}`,
        );
        return [];
      } catch (err) {
        console.error(
          `[Requirements Fetcher] Exception fetching assessment_conditions:`,
          err,
        );
        return [];
      }
    case "assessment_instructions":
      ...
      try {
        console.log(
          `[Requirements Fetcher] Fetching assessment_instructions from UnitOfCompetency for ${unitCode}`,
        );
        ...
        console.warn(
          `[Requirements Fetcher] No assessment instructions found in UnitOfCompetency for ${unitCode}`,
        );
        return [];
      } catch (err) {
        console.error(
          `[Requirements Fetcher] Exception fetching assessment_instructions:`,
          err,
        );
        return [];
      }
    ...
    default:
      if (!requirementTable || !type) {
        console.warn(
          `[Requirements Fetcher] Unknown validation type: ${validationType}`,
        );
        return [];
      }
      ...
      try {
        ...
        if (error) {
          console.error(
            `[Requirements Fetcher] Error fetching from ${requirementTable}:`,
            error,
          );
          return [];
        }
        ...
        if (data && data.length > 0) {
          console.log(
            `[Requirements Fetcher] Found ${data.length} requirements in ${requirementTable} for ${unitCode}`,
          );
          requirements = data.map((row: any) =>
            normalizeRequirement(row, type!),
          );
        } else {
          console.warn(
            `[Requirements Fetcher] No requirements found in ${requirementTable} for ${unitCode}`,
          );
        }
      } catch (err) {
        console.error(
          `[Requirements Fetcher] Exception fetching requirements:`,
          err,
        );
      }
      break;
...
export async function fetchAllRequirements(
    ...
    try {
      ...
      if (unitLink) {
        ...
        if (error && error.code === "42703") {
          console.log(
            `[Requirements Fetcher] Table ${table} doesn't have unit_url, trying unitCode...`,
          );
      ...
      if (error) {
        console.error(
          `[Requirements Fetcher] Error fetching from ${table}:`,
          error,
        );
        continue;
      }
      ...
      if (data && data.length > 0) {
        ...
        console.log(
          `[Requirements Fetcher] Found ${data.length} ${type} requirements for ${unitCode}`,
        );
      }
    } catch (err) {
      console.error(
        `[Requirements Fetcher] Exception fetching from ${table}:`,
        err,
      );
    }
  }
  ...
  try {
    console.log(
      `[Requirements Fetcher] Fetching assessment_conditions and assessment_instructions from UnitOfCompetency for ${unitCode}`,
    );
    ...
    if (uocError) {
      ...
      if (uocData.ac) {
        ...
        console.log(
          `[Requirements Fetcher] Found assessment_conditions from UnitOfCompetency for ${unitCode}`,
        );
      }
      ...
      if (uocData.ac || uocData.epc) {
        ...
        console.log(
          `[Requirements Fetcher] Found assessment_instructions from UnitOfCompetency for ${unitCode}`,
        );
      }
    } else {
      console.warn(
        `[Requirements Fetcher] No data found in UnitOfCompetency for ${unitCode}`,
      );
    }
  } catch (err) {
    console.error(
      `[Requirements Fetcher] Exception fetching from UnitOfCompetency:`,
      err,
    );
  }
  ...
  console.log(
    `[Requirements Fetcher] Added ${hardcodedCriteria.length} hardcoded compliance criteria`,
  );

  console.log(
    `[Requirements Fetcher] Total requirements fetched: ${allRequirements.length}`,
  );
 ...
 */
async function updateValidationTotal(
  supabase: any,
  validationDetailId: number,
  total: number,
) {
  try {
    console.log(
      `[Requirements Fetcher] Updating validation_total (${total}) for ID: ${validationDetailId}`,
    );
    ...
    if (error) {
      console.error(
        `[Requirements Fetcher] Failed to update validation_total:`,
        error.message,
      );
    }
  } catch (err: any) {
    console.error(
      `[Requirements Fetcher] Error in updateValidationTotal:`,
      err.message,
    );
  }
}
  ...
  switch (type) {
    case "knowledge_evidence":
      text =
        ...
        "";
      number =
        row.ke_number || row.requirement_number || row.number || String(row.id);
      description = text;
      break;
    ...
    case "performance_evidence":
      text =
        ...
        "";
      number =
        row.pe_number || row.requirement_number || row.number || String(row.id);
      description = text;
      break;
    ...
    case "foundation_skills":
      text =
        ...
        "";
      number =
        row.fs_number || row.skill_category || row.number || String(row.id);
      description = text;
      break;
    ...
    case "elements_performance_criteria":
      ...
      number =
        row.epc_number || row.element_number || row.number || String(row.id);
    ...
    case "assessment_conditions":
      ...
      number =
        row.ac_number || row.condition_number || row.number || String(row.id);
      description = text;
      break;
...
/**
 * Get hardcoded assessment instruction criteria
 * Assessment Conditions (AC) are now dynamically fetched from UnitOfCompetency.ac
 * Assessment Instructions (AI) are kept here as they are not captured in the n8n requirements scrape
 */
export function getHardcodedComplianceCriteria(
  unitCode: string,
): Requirement[] {
  const assessmentInstructions: Requirement[] = [
    {
      ...
      type: "assessment_instructions",
      description:
        "Assessment methods: Evaluate whether assessment methods are appropriate, varied, and aligned with unit requirements.",
    },
    {
      id: 8000002,
      unitCode,
      ...
      type: "assessment_instructions",
      description:
        "Evidence requirements: Verify that evidence requirements are clearly documented and aligned with assessment methods.",
    },
    {
      id: 8000003,
      unitCode,
      ...
      type: "assessment_instructions",
      description:
        "Clarity and language: Assess whether instructions are clear, unambiguous, and appropriate for the target audience.",
    },
    {
      id: 8000004,
      unitCode,
      ...
      type: "assessment_instructions",
      description:
        "Consistency: Check for consistency in language, terminology, and instructions across all assessment documents.",
    },
    {
      id: 8000005,
      unitCode,
      ...
      type: "assessment_instructions",
      description:
        "Assessment review process: Verify that feedback and review processes are clearly documented and embedded in the assessment.",
    },
    {
      id: 8000006,
      unitCode,
      ...
      type: "assessment_instructions",
      description:
        "Reasonable adjustments: Check whether guidance for reasonable adjustments is provided for learners with special needs.",
    },
    {
      id: 8000007,
      unitCode,
      ...
      type: "assessment_instructions",
      description:
        "Resubmission and reassessment policy: Verify that policies for resubmission and reassessment are clearly documented.",
    },
    {
      id: 8000008,
      unitCode,
      ...
      type: "assessment_instructions",
      description:
        "Compliance report: Overall assessment of compliance with RTO standards and identification of improvement areas.",
    },
  ];
 ...
 */
export function formatRequirementsByTypeAsJSON(
