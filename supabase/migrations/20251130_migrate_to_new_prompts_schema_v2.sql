...
-- ============================================================================
-- STEP 0: Drop any conflicting views or dependencies
-- ============================================================================

-- Drop any views that might depend on the prompt table
DO $$
BEGIN
    -- Add any view drops here if needed
    RAISE NOTICE 'Checking for conflicting dependencies...';
END $$;
...
BEGIN
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'prompt') THEN
        -- Check if prompt_deprecated already exists
        IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'prompt_deprecated') THEN
            -- Drop the old backup
            DROP TABLE prompt_deprecated CASCADE;
            RAISE NOTICE 'Dropped existing prompt_deprecated table';
        END IF;
        
        -- Rename to prompt_deprecated for backup
        ALTER TABLE prompt RENAME TO prompt_deprecated;
...
-- ============================================================================
-- STEP 2: Drop and recreate 'prompts' table to ensure clean schema
-- ============================================================================

-- Drop existing prompts table if it exists (to ensure clean slate)
DROP TABLE IF EXISTS prompts CASCADE;

-- Create new prompts table with correct schema
...
CREATE INDEX idx_prompts_default
  ON prompts(prompt_type, requirement_type, document_type, is_default)
...
BEGIN
    IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'prompt_deprecated') THEN
        ...
        RAISE NOTICE 'Migration complete: New "prompts" table created (empty), % records in "prompt_deprecated"', deprecated_count;
    ELSE
        RAISE NOTICE 'Migration complete: New "prompts" table created (empty)';
    END IF;
    
    RAISE NOTICE 'Next step: Run the seed script (20250129_seed_prompts.sql) to populate default prompts';
END $$;
...
-- 1. DROP TABLE prompts CASCADE;
-- 2. ALTER TABLE prompt_deprecated RENAME TO prompt;
