/**
 * Assessment and Learner Guide Report Generator with Storage
 * 
 * Enhanced version that:
 * 1. Generates Excel reports from validation_results data
 * 2. Uploads generated reports to Supabase storage
 * 3. Returns storage metadata for UI updates
 */

import ExcelJS from 'exceljs';
import { ValidationEvidenceRecord } from '../types/rto';
import { uploadReportToStorage, type StoredReport } from './reportStorageService';
...
export async function generateAndStoreAssessmentReport(
  ...
  try {
    ...
    console.error('[generateAndStoreAssessmentReport] Error:', errorMsg);
    return {
      success: false,
      error: errorMsg,
    };
...
/**
 * Generate Learner Guide Report and store in Supabase
 */
export async function generateAndStoreLearnerGuideReport(
  params: AssessmentReportParams
): Promise<GenerateAndStoreReportResult> {
  try {
    // Separate knowledge and performance evidence
    ...
    const errorMsg = error instanceof Error ? error.message : 'Unknown error';
    console.error('[generateAndStoreLearnerGuideReport] Error:', errorMsg);
    return {
      success: false,
      error: errorMsg,
    };
