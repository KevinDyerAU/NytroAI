...
export function SignUpDialog({ open, onOpenChange, onSwitchToLogin }: SignUpDialogProps) {
  ...
  useEffect(() => {
    const loadRtos = async () => {
      try {
        console.log('[SignUpDialog] Loading RTOs from Supabase...');
        const fetchedRtos = await fetchRTOsFromSupabase();
        console.log('[SignUpDialog] RTOs loaded successfully:', fetchedRtos.length);
        setRtos(fetchedRtos);
        ...
        console.error('[SignUpDialog] Failed to load RTOs:', errorMsg);
    ...
    if (open && rtos.length === 0) {
      loadRtos();
    }
  }, [open, rtos.length, rtoCode]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto bg-white">
        <DialogHeader>
          ...
          <DialogDescription>
            Join Nytro and streamline your RTO assessment validation
          </DialogDescription>
        </DialogHeader>
