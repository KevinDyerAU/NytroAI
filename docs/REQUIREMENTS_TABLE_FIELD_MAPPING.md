...
CREATE TABLE public.knowledge_evidence_requirements (
  unit_url text null,
  knowled_point text null,             -- ✅ THIS IS THE TEXT FIELD (note: typo in database)
...
**Field Mapping:**
- `number` → `ke_number`
- `text` → `knowled_point` (note: database has typo, not 'knowledge_point')
- `description` → `knowled_point` (same as text)
...
CREATE TABLE public.performance_evidence_requirements (
  unit_url text null,
  performance_evidence text null,      -- ✅ THIS IS THE TEXT FIELD
  created_at timestamp with time zone null default now(),
  id bigint generated by default as identity not null,
...
**Field Mapping:**
- `number` → `pe_number`
- `text` → `performance_evidence` (not 'performance_task')
- `description` → `performance_evidence` (same as text)
...
CREATE TABLE public.foundation_skills_requirements (
  unit_url text null,
  skill_point text null,               -- ✅ THIS IS THE TEXT FIELD
  created_at timestamp with time zone null default now(),
  id bigint generated by default as identity not null,
...
**Field Mapping:**
- `number` → `fs_number`
- `text` → `skill_point` (not 'skill_description')
- `description` → `skill_point` (same as text)
...
|-------|--------------|------------|-------------------|-------------|
| `knowledge_evidence_requirements` | `ke_number` | `knowled_point` | `knowled_point` | `Knowledge Evidence` |
| `performance_evidence_requirements` | `pe_number` | `performance_evidence` | `performance_evidence` | `Performance Evidence` |
| `foundation_skills_requirements` | `fs_number` | `skill_point` | `skill_point` | `Foundation Skills` |
